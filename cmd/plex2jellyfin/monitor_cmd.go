...
func newMonitorCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "monitor",
		Short: "View activity logs from plex2jellyfin-daemon",
		Long: `View and analyze activity logs from the plex2jellyfin-daemon daemon.
...
Examples:
  plex2jellyfin monitor              # Show last 3 days
  plex2jellyfin monitor --days 7     # Show last 7 days
  plex2jellyfin monitor --details    # Show detailed JSON entries
  plex2jellyfin monitor --errors     # Show only failed operations
  plex2jellyfin monitor --method ai  # Filter by parse method`,
...
func runMonitor(days int, showDetails bool, filterMethod string, showErrors bool) error {
	...
	if _, err := os.Stat(activityDir); os.IsNotExist(err) {
		...
		fmt.Println("Activity logging starts when plex2jellyfin-daemon processes files.")
		return nil
	}
