...
import (
	...
	"github.com/Nomadcxx/plex2jellyfin/internal/validator"
...
func newRootCmd() *cobra.Command {
	rootCmd := &cobra.Command{
		...
		SilenceUsage: true,
		Long: `Plex2Jellyfin helps review and clean up Jellyfin libraries.
...
Examples:
  plex2jellyfin organize /downloads/Silo.S02E02.mkv /media/TV
  plex2jellyfin organize /downloads/tv/ --library /media/TV
  plex2jellyfin organize /downloads/movies/ --library /media/Movies --dry-run`,
...
Examples:
  plex2jellyfin organize-folder /downloads/Movie.2024.1080p/ /media/Movies
  plex2jellyfin organize-folder /downloads/Show.S01E05/ /media/TV --keep-extras
  plex2jellyfin organize-folder /downloads/folder/ --dry-run`,
		Args: cobra.RangeArgs(1, 2),
...
Examples:
  plex2jellyfin validate /media/TV/Silo/
  plex2jellyfin validate /media/Movies/ --recursive`,
...
disk issues, or other problems that Plex2Jellyfin can handle.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := getSonarrClient(cmd)
			if err != nil {
				return err
...
Examples:
  plex2jellyfin watch /downloads/tv --tv-library /media/TV
  plex2jellyfin watch /downloads --tv-library /media/TV --movie-library /media/Movies
  plex2jellyfin watch /downloads -n  # dry-run mode`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load()
			if err != nil {
