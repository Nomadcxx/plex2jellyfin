...
func newScanCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "scan",
		Short: "Scan libraries and populate the Plex2Jellyfin database",
		Long: `Scan all configured libraries and populate the database.

This command scans your TV and movie libraries to build the Plex2Jellyfin database,
...
Examples:
  plex2jellyfin scan                    # Scan filesystem only
  plex2jellyfin scan --sonarr --radarr  # Also sync from Sonarr and Radarr
  plex2jellyfin scan --analyze          # Scan, then analyze duplicates/scattered media
  plex2jellyfin scan --json             # Emit machine-readable scan summary
  plex2jellyfin scan --stats            # Show database stats after scan`,
...
func runScan(syncSonarr, syncRadarr, syncFilesystem, showStats bool, scanPath string, analyze bool, jsonOutput bool) error {
	...
	if scanResult != nil {
		...
		if !jsonOutput && aiHelper != nil && (scanResult.AITriggered > 0 || scanResult.NeedsReview > 0) {
			...
			if scanResult.NeedsReview > 0 {
				fmt.Printf("Needs review: %d (run 'plex2jellyfin audit --generate' to review)\n", scanResult.NeedsReview)
			}
		}
	}
	...
	if showStats {
		...
		if shouldRunPostScanAnalysis(showStats, analyze) {
			...
			fmt.Println("\nPost-scan duplicate/scattered analysis skipped. Run 'plex2jellyfin scan --analyze' or targeted commands when needed.")
		}
	}
...
func renderPostScanAnalysis(dupAnalysis *service.DuplicateAnalysis, scatterAnalysis *service.ScatteredAnalysis, hasDuplicates, hasScattered bool) {
	...
	if hasDuplicates || hasScattered {
		...
		if hasDuplicates {
			...
			fmt.Println("   plex2jellyfin duplicates generate   # Generate deletion plans")
			fmt.Println("   plex2jellyfin duplicates dry-run    # Preview plans")
			fmt.Println("   plex2jellyfin duplicates execute    # Execute plans")
		}
		...
		if hasScattered {
			...
			fmt.Println("   plex2jellyfin consolidate generate  # Generate move plans")
			fmt.Println("   plex2jellyfin consolidate dry-run   # Preview plans")
			fmt.Println("   plex2jellyfin consolidate execute   # Execute plans")
		}

		fmt.Println("\n3. Review files needing classification:")
		fmt.Println("   plex2jellyfin audit --generate      # Generate audit report")
