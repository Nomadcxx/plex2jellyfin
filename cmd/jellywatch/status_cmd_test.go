...
func TestDeploymentDriftWarnings(t *testing.T) {
	dir := t.TempDir()
	build := filepath.Join(dir, "jellywatch.new")
	live := filepath.Join(dir, "jellywatch.live")
...
func TestDeploymentDriftWarningsQuietForIdenticalOrMissingBuild(t *testing.T) {
	...
	warnings := deploymentDriftWarnings([]deploymentBinary{
		{Name: "jellywatch", BuildPath: build, LivePath: live},
		{Name: "jellywatchd", BuildPath: filepath.Join(dir, "missing"), LivePath: live},
	})
	if len(warnings) != 0 {
