...
func printAuditSummary(plan *plans.AuditPlan) {
	...
	if plan.Summary.FilesToRename > 0 {
		...
		fmt.Printf("💡 Next: run 'jellywatch audit --dry-run' to preview, then 'jellywatch audit --execute' to apply these renames\n")
	} else {
		fmt.Printf("💡 Next: run 'jellywatch audit --dry-run' to inspect skipped/manual-review items\n")
	}
}
...
func auditPlanWarning(plan *plans.AuditPlan, cfg *config.Config) string {
	...
	if len(models) == 0 {
		return "Warning: existing audit plan contains only AI errors and no actions. Regenerate it with: jellywatch audit --generate"
	}

	currentModel := strings.TrimSpace(cfg.AI.Model)
	for model := range models {
		if currentModel != "" && model != currentModel {
			return fmt.Sprintf("Warning: existing audit plan contains only AI errors for model %s, but current config uses %s. Regenerate it with: jellywatch audit --generate", model, currentModel)
		}
	}
